...
public class ArrayListExample {
     ...
     */
    public static <T extends TwoDShape<T>> double total_area(ArrayList<T> twoDList){
        Iterator<T> iter = twoDList.iterator();
        double totalArea = 0;
        while(iter.hasNext())
            totalArea += (iter.next()).area();
     ...
     */
    public static <T extends Rectangle> double total_perimeter(ArrayList<T> rectList){
        Iterator<T> iter = rectList.iterator();
        double totalPerimeter = 0;
        while(iter.hasNext()){
            totalPerimeter += iter.next().perimeter();
        }
     ...
     */
    public static <T extends GeometricShape<T>> void describe_all(ArrayList<T> shapeList){
        Iterator<T> iter = shapeList.iterator();
        while(iter.hasNext())
            iter.next().describe();
     ...
     *      {@code new Sphere(0.0);          }
     */
    public static void add_empties(ArrayList<GeometricShape<?>> shapeList){
        shapeList.add(new Circle(0.0));
        shapeList.add(new Cone(0.0, 0.0));
        shapeList.add(new Rectangle(0.0, 0.0));
        shapeList.add(new Sphere(0.0));

    }

    public static <T extends GeometricShape<T>> ArrayList<T> supersize_list(ArrayList<T> shapeList){
        ArrayList<T> doubledList = new ArrayList<T>();
        Iterator<T> iter = shapeList.iterator();

        while(iter.hasNext()){
            doubledList.add(iter.next().supersize());
        }
        return doubledList;
    }
